...
/////////////////////education///////
    ...
    $stmt = $pdo->query("SELECT year , name FROM Education JOIN Institution ON Education.institution_id=Institution.institution_id WHERE profile_id = '$_GET[profile_id]' ORDER BY rank ");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

    echo"<p>Education</p>";
    echo "<ul>";
    foreach ($rows as $row) {
      echo "<li>".$row['year']." : ".$row['name']."</li>"."<br>"  ;
    }
    echo "</ul>";
